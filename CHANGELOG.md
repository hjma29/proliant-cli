...
- `proliant oneview update enclosure`: proceeding through OneView's non-disruptive-fabric validation warning now matches the GUI's "OK to proceed" exactly — it clears only the validation guard (`validateIfLIFirmwareUpdateIsNonDisruptive`) and does **not** also force-reinstall firmware. Previously, confirming the warning conflated the two and always set `forceInstallFirmware` as well. `--force` still independently forces a reinstall *and* bypasses the guard up front; the two are now separate knobs, so a plain "proceed anyway" behaves exactly like clicking OK in the GUI.
