...
- `proliant com workspaces list` / `proliant com workspace use`: a workspace created or joined in HPE GreenLake *after* the last `proliant com login` didn't show up, since both commands only ever consulted the workspace list cached at login time. `workspaces list` now refreshes live from GreenLake by default (falling back to the cached list if offline or on an `--api-client`-only session), and `workspace use <name>` automatically refreshes and retries if the target workspace isn't in the cached list — no re-login required.
