...
### Bug Fixes
- `proliant oneview interconnects describe`: fixed a misleading "Firmware baseline" line that showed the logical enclosure's *assigned/target* SSP as if it were already installed, even when the update had actually failed or was blocked. Now always shows the same three plain fields as the GUI's own "General" page — "Firmware baseline", "Firmware version from baseline", and "Installed firmware version" — with no separate "target"/"not yet applied" concept invented on top; any mismatch between the baseline's version and what's installed is simply visible by comparing those two lines, exactly like the GUI.
