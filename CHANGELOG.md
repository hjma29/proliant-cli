...
### Bug Fixes
- `proliant oneview interconnects describe`: fixed a misleading "Firmware baseline" line that showed the logical enclosure's *assigned/target* SSP as if it were already installed, even when the update had actually failed or was blocked. Now shows "Installed SPP" (read from the logical interconnect's own firmware status, the same source the GUI's "Installed:" column uses) separately from "Target baseline", and only shows the target line — marked "(not yet applied)" — when it actually differs from what's installed.
