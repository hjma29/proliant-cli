...
Credentials can be supplied via (in priority order):
  ...
  3. Credentials file: ~/.config/pcli/com/credentials.yml
...
class COMSession:
    ...
    def from_user_token(cls) -> "COMSession":
        """Load cached user OAuth token from a previous 'pcli com login'.
        ...
        """
        from pcli.com.login import load_token  # avoid circular import

        data = load_token()
        if not data:
            raise CredentialsError(
                "Not logged in. Run 'pcli com login' first."
            )
        ...
        if data.get("expires_at", 0) <= time.time() + 60 and not data.get("refresh_token"):
            raise CredentialsError(
                "Session expired. Run 'pcli com login' to re-authenticate."
            )
    ...
    def load(cls, client_id: Optional[str] = None,
        ...
        this and trigger the credentials wizard (pcli com login).
        """
    ...
    async def ensure_token(self, client: httpx.AsyncClient) -> str:
        ...
        async with self._get_lock():
            ...
            try:
                from pcli.com.login import cache_glp_token
    ...
    async def _do_refresh(self) -> None:
        ...
        from pcli.com.login import refresh_token_if_needed
