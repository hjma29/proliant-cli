...
from pcli.com.auth import REGION_MAP, CredentialsError
...
# Constants
# ---------------------------------------------------------------------------
CREDS_FILE        = Path.home() / ".config" / "hpecom" / "credentials.yml"
TOKEN_CACHE       = Path.home() / ".config" / "hpecom" / "token.json"
...
def save_token(token: dict, region: str = "us-west",
    ...
    """Persist the user OAuth token to ~/.config/pcli/com/token.json."""
...
GLP_CRED_NAME_PREFIX = "GLP-pcli-com-temp"
...
async def _cleanup_stale_pcli_credentials(access_token: str, ccs_session: str) -> None:
...
async def create_glp_api_credential(access_token: str, ccs_session: str, workspace_id: str) -> Optional[dict]:
    ...
    await _cleanup_stale_pcli_credentials(access_token, ccs_session)
...
async def get_glp_api_token(client_id: str, client_secret: str, workspace_id: str) -> Optional[str]:
    ...
    data = load_token()
    if not data:
        raise CredentialsError("Not logged in. Run 'pcli com login' first.")

    workspaces = data.get("workspaces", [])
    if not workspaces:
        raise CredentialsError(
            "No workspace list cached. Run 'pcli com login' to refresh."
        )
    ...
    if not target:
        ...
        raise ValueError(
            ...
            f"Run 'pcli com get workspaces' to see the full list."
        )
            ...
            async with httpx.AsyncClient(follow_redirects=True, timeout=20) as client:
                ...
                if "redirect-idp" in remediations:
                    raise AuthFlowError(
                        ...
                        "Use 'pcli com login' with Okta Verify instead."
                    )
