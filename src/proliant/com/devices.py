...
from proliant.com.auth import COMSession, AuthError
...
async def fetch_devices(
    ...
    stay logged in without running 'proliant com login' again.
...
async def add_compute_devices(
    ...
    """
    from proliant.com.login import load_token, get_glp_api_token

    data = load_token()
    if not data:
        raise PermissionError("Not logged in. Run 'proliant com login' first.")
    ...
    if not glp_client_id or not glp_client_secret:
        raise PermissionError(
            "No GLP API credential found. Run 'proliant com login' to create one.\n"
    ...
    if not glp_token:
        raise PermissionError(
            ...
            "run 'proliant com login' to create a new one."
        )
