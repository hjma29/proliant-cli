...
async def fetch_storage_report_data(client: ILOClient) -> list[dict]:
    ...
    Used by both `servers list` (rolled up into a compact summary), `servers
    describe` and `storage describe` (shown as a full per-drive table).
    Each entry represents one Redfish Storage subsystem and classifies its
...
async def fetch_server_list_info(client: ILOClient) -> list[tuple[str, str]]:
    """Fetch server identity + iLO IP for the 'proliant ilo servers list' table."""
