...
import proliant.ilo.inventory as inventory
...
from proliant.common.display import print_json
...
def _single_line_cell(value: object) -> str:
    """Normalize control characters so plain-text tables stay on one terminal row."""
    return " ".join(str(value).splitlines()) if value is not None else "N/A"
...
def print_fleet_table(results: list[tuple[str, str | None, list]],
    ...
    for host_name, error, rows in results:
        if error:
            ...
            server_data[host_name]["_error"] = _single_line_cell(error)
        else:
            server_data[host_name] = {k: _single_line_cell(v) for k, v in rows}
...
def print_storage_table(results: list[tuple[str, str | None, list]]) -> None:
    ...
    RAID Attached | Direct Attached.

    Delegates to proliant.common.display.print_storage_list_table, which
    `oneview`/`com` storage list also use, so all three modules render
    identical output.
    """
...
def print_network_host_table(results: list[tuple[str, str | None, list]]) -> None:
    """`proliant ilo network list`: Server | Location | Port | MAC |
    Link Status.

    Delegates to proliant.common.display.print_network_list_table, which
    `oneview`/`com` network list also use, so all three modules render
    identical output.
    """
