...
from proliant.ilo.printers import _print_fw_components, _print_fw_queue
...
async def run_fw_upgrade(
    ...
    *,
    ...
    from proliant.ilo.power import reset_server
        ...
        if needs_host_restart:
            ...
            if answer == "y":
                ...
                console.print(
                    ...
                    f"  • Run [bold]proliant ilo firmware queue {host['name']}[/bold] to check queue status"
                )
        else:
            console.print(
                f"\n[bold yellow]Updates queued.[/bold yellow] Reboot [bold]{host['name']}[/bold] to apply.\n"
