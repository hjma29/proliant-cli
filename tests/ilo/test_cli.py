...
def test_parser_show_subcommands():
    """All 'show' subcommands must be recognised."""
    from pcli.ilo.cli import _build_parser
    parser = _build_parser()
    for what in ["fleet", "ilo", "network", "nic", "storage",
                 "cpu", "memory", "com", "full", "disk-map"]:
        args = parser.parse_args(["show", what])
...
def test_parser_show_host_filter():
    from pcli.ilo.cli import _build_parser
    parser = _build_parser()
    args = parser.parse_args(["show", "ilo", "--host", "dl325-gen12"])
...
def test_parser_show_raw_flag():
    from pcli.ilo.cli import _build_parser
    parser = _build_parser()
    args = parser.parse_args(["show", "nic", "--raw"])
