"""
CLI-level JSON output tests for pcli oneview --json.
"""
...
class TestOneviewParserJson:
    def test_parser_accepts_json_flag(self):
        from pcli.oneview.cli import _build_parser
        parser = _build_parser()
        args = parser.parse_args(["--json", "list", "servers"])
        assert args.json_output is True

    def test_parser_json_default_false(self):
        from pcli.oneview.cli import _build_parser
        parser = _build_parser()
        args = parser.parse_args(["list", "servers"])
        assert args.json_output is False

    def test_parser_json_on_list_networks(self):
        from pcli.oneview.cli import _build_parser
        parser = _build_parser()
        args = parser.parse_args(["--json", "list", "networks"])
        assert args.json_output is True


class TestOneviewJsonServers:
    def test_list_servers_json_is_valid(self, capsys):
        from pcli.oneview import cli

        with patch("pcli.oneview.cli._load_client", return_value=_make_mock_client()), \
             patch("pcli.oneview.servers.list_servers_with_profiles",
                   new_callable=AsyncMock, return_value=FAKE_SERVERS):
            cli.main(["--json", "list", "servers"])
...
class TestOneviewJsonNetworks:
    def test_list_networks_json_is_valid(self, capsys):
        ...
        with patch("pcli.oneview.cli._load_client", return_value=_make_mock_client()), \
             patch("pcli.oneview.network.list_networks",
                   new_callable=AsyncMock, return_value=FAKE_NETWORKS):
            cli.main(["--json", "list", "networks"])
